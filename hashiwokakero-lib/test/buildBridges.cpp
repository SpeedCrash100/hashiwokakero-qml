...
class BuildBridgesTest : public ::testing::Test {
 protected:
  ...
  void SetUp() override {
    ...
    board.m_islands = {
        {BoardPosition(0, 0), 3}, {BoardPosition(0, 4), 2},
        {BoardPosition(0, 6), 2}, {BoardPosition(1, 2), 3},
        {BoardPosition(1, 5), 1}, {BoardPosition(3, 0), 6},
        {BoardPosition(3, 2), 6}, {BoardPosition(3, 6), 3},
        {BoardPosition(6, 0), 4}, {BoardPosition(6, 2), 4},
        {BoardPosition(6, 4), 3}, {BoardPosition(6, 6), 3},
...
TEST_F(BuildBridgesTest, RequirementUpdateCheck) {
  ...
  ASSERT_TRUE(
      board.tryBuildBridge({BoardPosition(1, 2)}, {BoardPosition{1, 5}}));
  ...
  auto it =
      std::find(islands.begin(), islands.end(), Island(BoardPosition(1, 2)));
...
TEST_F(BuildBridgesTest, BridgeUpgrade) {
  ...
  ASSERT_TRUE(
      board.tryBuildBridge({BoardPosition(3, 0)}, {BoardPosition{3, 2}}));
  ASSERT_TRUE(
      board.tryBuildBridge({BoardPosition(3, 0)}, {BoardPosition{3, 2}}));
  ...
  auto it =
      std::find(islands.begin(), islands.end(), Island(BoardPosition(3, 0)));
  ASSERT_NE(it, islands.end());
...
TEST_F(BuildBridgesTest, BridgeCollide) {
  ...
  ASSERT_TRUE(
      board.tryBuildBridge({BoardPosition(1, 2)}, {BoardPosition{1, 5}}));
  ASSERT_FALSE(
      board.tryBuildBridge({BoardPosition(0, 4)}, {BoardPosition{6, 4}}));
  ASSERT_EQ(board.bridges().size(), 1);
}
